...
def load_data(csv_file):
    ...
    for index, row in data_df.iterrows():
        ...
        left = fix_path(row['left'])
        right = fix_path(row['right'])
        angle = row['steering']

        if os.path.exists(center):
            data.append((center, angle))
        if os.path.exists(left):
            data.append((left, angle + 0.2))  
        if os.path.exists(right):
            data.append((right, angle - 0.2)) 
...
def preprocess_image(image):
    image = image[60:135, :, :]  # Crop sky and hood
...
def load_batch(image_paths, steerings):
    images = []
    angles = []
    for i in range(len(image_paths)):
        ...
        images.append(image)
        angles.append(steerings[i])
    return np.array(images), np.array(angles)

# Nvidia-style model WITHOUT Dropout and Lambda
def nvidia_model():
    model = Sequential([
...
def train_model():
    ...
    X_train, y_train = load_batch(image_paths_train, steerings_train)
    X_val, y_val = load_batch(image_paths_val, steerings_val)
